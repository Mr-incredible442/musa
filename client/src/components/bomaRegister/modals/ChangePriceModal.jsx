...
import { BomaStoreContext } from '../../../context/BomaStoreContext';
import { ShopCContext } from '../../../context/ShopCContext';

import { SHOPC_URL, BOMA_STORE_URL } from '../../../helpers/variables';

function ChangePriceModal() {
  const { storeShift } = useContext(BomaStoreContext);
  const { shopCShift } = useContext(ShopCContext);

  const [products, setProducts] = useState([]);

  useEffect(() => {
    const restaurantStock = Array.isArray(shopCShift?.stock)
      ? shopCShift.stock.map((item) => ({
          ...item,
          source: 'restaurant',
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedProduct.source === 'store') {
      requestUrl = `${BOMA_STORE_URL}/${selectedProduct.shiftId}/updatename/${selectedProduct._id}`;
    } else {
      let baseUrl;
