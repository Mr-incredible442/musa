...
import Register from '../model/BomaRegister.schema.js';
...
router.post('/:id/addtostock', async (req, res) => {
  try {
    ...
    req.app.get('io').emit('bomaregister', updatedShift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.post('/newshift', async (req, res) => {
  try {
    ...
    res.json(newShift);
    req.app.get('io').emit('bomaregister', newShift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.json(lastShift);
    req.app.get('io').emit('bomaregister', lastShift);
  } catch (error) {
