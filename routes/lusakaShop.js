...
import LusakaShop from '../model/LusakaShop.schema.js';

// get all shifts
router.get('/shifts', async (req, res) => {
  try {
    const shift = await LusakaShop.find();
...
router.get('/paginatedshifts/:year?/:month?', async (req, res) => {
  try {
    ...
    if (year && month) {
      ...
      const shifts = await LusakaShop.find({
        date: { $gte: startOfMonth, $lte: endOfMonth },
      });

      // Aggregate to get distinct years and months in the database
      const distinctYearsMonths = await LusakaShop.aggregate([
        {
          $match: {
...
router.get('/', async (req, res) => {
  try {
    const shift = await LusakaShop.findOne().sort({ _id: -1 }).limit(1);

    req.app.get('io').emit('lusakashop', shift);
    res.json(shift);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.get('/:id', async (req, res) => {
  try {
    const shift = await LusakaShop.findOne({ _id: req.params.id });
...
router.post('/:id/addtostock', async (req, res) => {
  try {
    ...
    const maxCodeResult = await LusakaShop.aggregate([
    ...
    const generatedCode =
      maxCodeResult.length === 0 || maxCodeResult[0].maxCode === undefined
        ? 55000
        : maxCodeResult[0].maxCode + 1;

    const updatedShift = await LusakaShop.findByIdAndUpdate(
      id,
      {
        $push: {
          stock: {
    ...
    if (updatedShift.status === 'current') {
      req.app.get('io').emit('lusakashop', updatedShift);
    }
...
router.post('/:shiftId/editstock/:stockId', async (req, res) => {
  try {
    ...
    const updatedShift = await LusakaShop.findOneAndUpdate(
    ...
    if (!updatedShift) {
      return res
        .status(404)
        .json({ message: 'LusakaShop or stock item not found' });
    }
...
router.post('/newshift', async (req, res) => {
  try {
    const lastShift = await LusakaShop.findOne().sort({ _id: -1 }).limit(1);
    ...
    const newShift = new LusakaShop(clonedShift);
    await newShift.save();

    res.json(newShift);
    req.app.get('io').emit('lusakashop', newShift);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedShift = await LusakaShop.findByIdAndDelete(req.params.id);

    if (!deletedShift) {
      return res.status(404).json({ message: 'LusakaShop not found' });
    }
    ...
    if (lastShift) {
      ...
      await lastShift.save();
      req.app.get('io').emit('lusakashop', lastShift);
    }

    res.json({ message: 'LusakaShop deleted successfully' });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
