...
import { LUSAKA_SHOP_URL } from '../helpers/variables';

export const LusakaShopContext = createContext();
const lusakaShopReducer = (state, action) => {
  switch (action.type) {
    case 'SET_SHIFT':
      return {
...
export const LusakaShopContextProvider = ({ children }) => {
  const [shift, dispatch] = useReducer(lusakaShopReducer, { shift: null });
  const [updatedStock, setUpdatedStock] = useState({});

  useEffect(() => {
    apiCall.get(LUSAKA_SHOP_URL).then((response) => {
  ...
  return (
    <LusakaShopContext.Provider value={{ shift: updatedStock, dispatch }}>
      {children}
    </LusakaShopContext.Provider>
  );
};
